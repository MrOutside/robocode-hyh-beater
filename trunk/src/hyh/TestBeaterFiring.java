...
/**
 * This tests for variability in bullet power administered by the robot Beater.
 *
 * @author Yong Hong Hsu
 */
public class TestBeaterFiring extends RobotTestBed {

  // True if Beater has fired a bullet with power less than 1.4;
  boolean firePowerReachLevelOne = false;
  // True if Beater has fired a bullet with power between 1.5 to 1.9;
  boolean firePowerReachLevelTwo = false;
  // True if Beater has fired a bullet with power between 2.0 to 2.4;
  boolean firePowerReachLevelThree = false;
  // True if Beater has fired a bullet with power greater than 2.4;
   ...
   * that Beater is using proportional firing based on distance of detected enemy robot. 
   * 
   * @param event Info about the current state of the battle.
   */
  @Override public void onTurnEnded (TurnEndedEvent event) {
    
    // All active bullets belong to Beater since SittingDuck does not fire.
  ...
  /**
   * After running all matches, determine if Beater has had variability in its bullet power.
   * 
